...
attempts to build them, and possibly executes them. If the applications are tested, a test report is
generated.
...
> be able to find the board metadata and thus won't build or run tests for it. This is a mistake that often happens with the custom boards.
...
Twister has a lot of command line options that can be used to control its behaviour. Run
